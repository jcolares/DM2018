...
plot.hist <-  function(series, label_series = "", label_x = "", label_y = "", colors = NULL, bin = NULL, title=NULL) {
...
plot.density <-  function(series, label_series = "", label_x = "", label_y = "", colors = NULL, bin = NULL) {
  if("variable" %in% colnames(series)) {
    ...
    if (is.null(bin)) 
      ...
      grf <- grf + geom_density(adjust = bin)
  }  
  else {
    grf <- ggplot(series, aes(x=value))
    if (is.null(bin)) {
      ...
      if (!is.null(colors)) 
        grf <- grf + geom_histogram(adjust = bin,fill=colors)
      else
        grf <- grf + geom_histogram(adjust = bin)
    }
  }
  grf <- grf + theme_bw(base_size = 10)
  grf <- grf + xlab(label_x)
